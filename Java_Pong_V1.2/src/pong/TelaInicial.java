...
public class TelaInicial extends Tela implements Runnable, KeyListener {
    private int ix = 220; // Indicador X
    private int iy = 236; // Indicador Y
    private Map<Integer, Integer> vetor;
    private int posicao = 1;
    private JFrame jframe = new JFrame(); // Utilizado para abrir uma janela com Java
    private Jogo jogo1;
    private Derrota derrota;
    private Placares placares;
    private static boolean jogoBot = false;
    private static boolean jogo2Jogador = false;
    private static boolean telaDerrota = false;
    private static boolean telaPlacares = false;
    ...
    public void keyPressed(KeyEvent e) { // O que fazer se a tecla for apertada
        ...
        if (telaDerrota) {
            if (derrota.getTrigger1()) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    derrota.setTrigger1(false);
                }
            } else if (derrota.getTrigger2()) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    derrota.setTrigger2(false);
                }
            } else if (derrota.getTrigger3()) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    derrota.setTrigger3(false);
                }
            }
        }
        ...
        if ((jogoBot) || (jogo2Jogador)) {
            if (e.getKeyCode() == KeyEvent.VK_W) {
                Jogo.jogador1.setUp(true);
            }
            if (e.getKeyCode() == KeyEvent.VK_S) {
                Jogo.jogador1.setDown(true);
            }
        }

        if ((jogo2Jogador)) {
            if (e.getKeyCode() == KeyEvent.VK_UP) {
                Jogo.jogador2.setUp(true);
            }
            if (e.getKeyCode() == KeyEvent.VK_DOWN) {
                Jogo.jogador2.setDown(true);
            }
        }

        if ((telaDerrota)) {
            if (e.getKeyCode() == KeyEvent.VK_W) {
                if (derrota.getNumero() > 1) {
                    derrota.setNumero(derrota.getNumero()-1);
                }
            }
            if (e.getKeyCode() == KeyEvent.VK_S) {
                if (derrota.getNumero() < 26) {
                    derrota.setNumero(derrota.getNumero()+1);
                }
            }
        }
    ...
    public void keyReleased(KeyEvent e) { // O que fazer se a tecla for solta
        if ((jogoBot) || (jogo2Jogador)) {
            if (e.getKeyCode() == KeyEvent.VK_W) {
                Jogo.jogador1.setUp(false);
            }
            if (e.getKeyCode() == KeyEvent.VK_S) {
                Jogo.jogador1.setDown(false);
            }
        }

        if (jogo2Jogador) {
            if (e.getKeyCode() == KeyEvent.VK_UP) {
                Jogo.jogador2.setUp(false);
            }
            if (e.getKeyCode() == KeyEvent.VK_DOWN) {
                Jogo.jogador2.setDown(false);
            }
        }
    }
    ...
    public static boolean getJogoBot() {
        return jogoBot;
    }
    ...
    public static boolean getJogo2Jogador() {
        return jogo2Jogador;
    }
    ...
    public static boolean getTelaDerrota() {
        return telaDerrota;
    }
    ...
    public static boolean getTelaPlacares() {
        return telaPlacares;
    }
